...
sequence alignments) to place their experiments in a
...
of BITS outperforms existing approaches. We also demonstrate that the algorithm 
...
because each parallel thread performs roughly the same amount of work, the 
algorithm has little thread divergence.
...
approach is used by the UCSC Genome Browse, BEDTools~\citep{quinlan2010},
...
\emph{enumerated} in order to count the intersections.
...
\begin{figure*}[btp]
	\centering
	\includegraphics[width=6in]{figures/seq-counting.eps}
...
to derive the \emph{count}, BITS \emph{infer} the intersection count by 
...
\begin{center}
	...
	differing dataset sizes and simulation rounds. Timings in italics were 
	extrapolated owing to long run times.}
...
\begin{figure*}[t]
        \includegraphics[width=7in,height=8in]{figures/heatmap.eps}
