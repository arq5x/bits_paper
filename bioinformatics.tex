...
Ira Hall$^2$,
and
Aaron Quinlan$^3$\footnote{to whom correspondence should be addressed}}
...
scalable approach to interval set intersection. Our analyses
...
Units (GPUs). We illustrate the utility of this new algorithm for efficient
Monte-Carlo simulations that measure the significance of relationships between
...
straight-forward reduction to element uniqueness (which is known to be
$\Theta(N\log N)$~\citep{misra1982}).  We illustrate that, in the  sequential
...
(e.g., all known genes).  More generally, an interval $a$ is the set of all
numbers between a start value and an end value, which can be represented as the
pair $(a.start, a.end)$.  Two intervals $a$ and $b$ {\em intersect} when 
...
sets. There are four natural sub-problems for interval set intersection: 1) 
\emph{decision} - does there exist at least one 
interval in $A$ intersect an interval in $B$?; 2) {\em counting} - how 
many total pair-wise intersections exist between sets $A$ and $B$?; 3) {\em 
...
intersections between $A$ an $B$? While the algorithm we have devised solves all 
...
query interval to identify all of the intersecting database intervals. Formally,
...
\begin{figure}[h]
		...
		intersection to the BITS two binary search approach. \textbf{A}. Binary
		searches of database intervals sorted by start coordinate will
		occasionally identify overlapping intervals. However, because of 
		contained intervals, one does not know how far one must scan 
		the database to identify all intersections. \textbf{B}. Moreover,
		contained intervals often cause single binary searches to 
		falsely conclude that no intersections exist for a given query 
		interval.  \textbf{C}. To overcome these limitations, the BITS
		algorithm uses two binary searches of the database intervals: one
		into a sorted list of database end coordinates and the other into a
		sorted list of start coordinates. Each search excludes database
		intervals that \emph{cannot} intersect the query, leaving only those
		that database intervals that \emph{must} intersect the query.}
		\label{bitssearching}
\end{figure}
...
problems.  The size of $\mathcal{I}(B,a_i)$ can also serve as the previously
missing terminating condition in the enumeration problem.
...
determines the number of intervals in the database $B$ that intersect query
interval $a_i$.  As shown in Fig. 1C, the database $B$ is split into two integer
lists $B_S = [b_1.start, b_2.start, \dots, b_M.start]$ and $B_E = [b_1.end,
b_2.end, \dots, b_M.end]$, which are each sorted numerically in ascending
order.  Next, two binary searches are performed, 
...
(pseudocode for the \emph{decision}, \emph{per-interval counting}, and
\emph{enumeration} sub-problems can be found in the Supplemental Materials).
...
% \begin{enumerate}
	...
	($\langle star,end \rangle$), and
	input files are typically chromosomal intervals 
	($\langle chrom,start,end \rangle$), each
	file is first projected down to a one-dimensional generic interval.  This
	is a straight forward process that adds an offset associated with each
	chromosome to the start and end of
	...
	The total accumulated count $c$ is returned.
	\begin{algorithm}[h]
		\DontPrintSemicolon
		\footnotesize
...
The time complexity of the BITS algorithm is $O( (|A|+|B|) \log |B|)$, which can
be shown to be optimal by a straight-forward reduction to element uniqueness
(which is known to be $\Theta(N\log N)$~\citep{misra1982}).  To compute
...
Sorting on the GPU has been an active area of research for many years, and
current GPU sorting algorithms can sort billions of integers within
seconds~\citep{merrill2011,satish2009}.
...
approaches}
We implemented a sequential version of the BITS algorithm as a stand-alone C++
...
(Figure 2). The comparisons presented are based on sequence alignment intervals
extracted from datasets created for the CEU individual NA12878 by the 1000
Genomes Project~\citep{durbin2010}, as well as RefSeq exons (N=400,531). Owing
...
within the sets. For example, tree-based solutions (i.e., BEDTools and UCSC)
that place intervals into hierarchical bins may perform poorly when intervals
are unevenly distributed among the bins. To test the effect of differing
interval distributions on algorithm performance, we randomly sampled 1 and 10
million alignment intervals from both whole-genome and exome-capture datasets
for this individual (see Supplemental Methods). Each algorithm was evaluated
...
between exome-capture alignments and whole-genome alignments. Since the interval 
sets have a large number of intervals and each set has a different genomic 
...
databases of 1 million alignment intervals from each interval distribution.
	...
	deviation.  Gray bars reflect the portion of the run time consumed by
...
BEDTools and UCSC tree-based algorithms must \emph{enumerate} each intersection
in order to derive the \emph{count} of intersections, BITS can \emph{infer} the
intersection count
...
interval searches on these trees require a linear scan of each associated bin,
the number of intervals searched grows linearly with respect to the input size
(albeit with a very small coefficient).  In the worst-case, where all intervals
are in a single bin, a search would scan the full input set.  In contrast, BITS
is based on two binary searches so the number of operations is proportional to
$\log$ of the input size regardless of the input distribution.
...
sets by setting up their underlying data structures. The BITS setup process
...
end coordinates.  In contrast, the UCSC setup process places each chromosomal
interval into a hash table.  As shown in Figure~\ref{seq-counting}, data
...
task parallelism since each round is independent.  BITS running on CUDA goes a
step further and exposes fine-grain parallelism in both the setup step, with
...
dozen to thousands of experiments (e.g. 25,921 pairwise comparisons in
...
Due to the long runtimes, timings for sequential BITS (BITS-SEQ) and UCSC with
...
\begin{center}
	...
	\begin{table}[h!b!p!]
	\caption{Speed comparison for Monte Carlo simulations across
	differing dataset sizes and simulation rounds.}
	\begin{tabular}{l l c c c c}
	\multicolumn{2}{c}{} & \multicolumn{4}{c}{Time (s) for MC iterations} \\
	\cline{3-6}
...
%        
        ...
        % RESULTS: Applications for Monte-Carlo simulations 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Uncovering genomic relationships in large-scale datasets}
\label{hm:section}
...
functional and regulatory elements in the genome. To illustrate the 
potential that BITS-CUDA has for large-scale data mining experiments, 
we conducted an unbiased screen for significant relationships among
161 genome annotation tracks using Monte Carlo simulation (see
Supplemental Materials). This analysis was based upon several 
functional annotations collected by the ENCODE
project~\citep{encode2007} for the GM12878, H1-hESC, and K562 
cell lines, including assays for 24 transcriptions factors 
(often with replicates), 8 histone
modifications, open chromatin, and DNA methylation. We included all of
these assays for each cell line, as well as a wide range of existing
genome annotations from the UCSC genome browser (e.g, repeats, genes,
segmental duplications, and conserved regions).

Using BITS-CUDA, we compared the observed and expected number
of intersections for each of the 25,921 (i.e., 161*161) pairwise 
dataset relationships using 10000 Monte Carlo simulations (Figure 3).
...
consistent with previous descriptions of so-called ``hot regions''
...
\begin{figure*}[t]
        ...
        \caption[]{MC-CUDA measurements of spatial correlations
among 161 genome features from the ENCODE project and from the UCSC
Genome Browser.}
\end{figure*}
