...
its utility for efficient Monte Carlo \textcolor{red}{(MC)} simulations measuring the significance of 
...
comparison\textcolor{red}{. Moreover}, typical analyses require comparisons to many large sets of
...
divergence. \textcolor{red}{Although} thread divergence degrades performance on any architecture 
...
Interval intersection has many applications in genomics, \textcolor{red} {and
several}
...
hierarchical \textcolor{red} {``bins.''}  Intervals from a second dataset are then compared 
...
the number of intervals in each partition \textcolor{red}{is} unbalanced.  In the second case,
...
\begin{algorithm}[h]
	...
	\Begin {
		$first \gets \textsc{\textcolor{red}{BinarySearch}}(B_S, a.end)$\;
		$last \gets \textsc{\textcolor{red}{BinarySearch}}(B_E, a.start)$\;
		$c \gets first - last$ 
		\tcc*[f]{$=|B|-(last+(|B|-first))$}
...
use of \textcolor{red}{MC} simulations that compare \emph{observed} interval 
...
\textcolor{red}{MC} 
simulations measuring relationships between interval sets. As described,
...
algorithms~\citep{merrill2011,satish2009}.  \textcolor{red}{MC} simulations have obvious
...
We demonstrate the improved performance of BITS over UCSC for
\textcolor{red}{MC} 
...
BITS-SEQ is 12x faster \textcolor{red}{than UCSC}, and BITS-CUDA is 267x
...
\begin{center}
	\begin{table}[h!b!p!]
	\caption{Runtime (seconds) comparison for \textcolor{red}{MC} simulations. 
...
\label{hm:section}
The efficiency of BITS for MC applications on
